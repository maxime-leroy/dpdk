...
dpaa2_dev_rx_queue_intr_unbind(struct dpaa2_queue *dpaa2_q)
{
	...
	/* No portal access here: DEST_NONE above parks the FQ off its DPCON, so
	 * the channel can no longer become non-empty and cannot raise a CDAN.
	 * Disabling the CDAN and masking the portal DQRI are done on the polling
	 * lcore by rte_eth_dev_rx_intr_disable(), which the application must call
	 * before stop/close (see the ops contract). Driving the single-owner
	 * software portal from this control-plane path would race that worker.
	 */
